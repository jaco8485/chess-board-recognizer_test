...
def board_accuracy(board_one: torch.Tensor, board_two: torch.Tensor) -> float:
    ...
    if board_one.shape != board_two.shape:
        raise ValueError(
            f"Both boards must have the same shape. board_one shape: {board_one.shape}, board_two shape: {board_two.shape}"
        )
...
def per_piece_accuracy(predicted_board: torch.Tensor, true_board: torch.Tensor) -> dict:
    ...
    if predicted_board.shape != true_board.shape:
        raise ValueError(
            f"Both boards must have the same shape. predicted_board shape: {predicted_board.shape}, true_board shape: {true_board.shape}"
        )
